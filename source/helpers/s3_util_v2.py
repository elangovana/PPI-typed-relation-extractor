...
class S3Util:
    def uploadfile(self, localpath, remote_path, quite_mode=False):
        """
...
Lists the files in s3
    ...
    def upload_files(self, local_dir, remote_path, num_threads=20, quite_mode=False):
        """
Uploads the files in local directory to s3
        ...
        input_tuples = ((f, rp(f, remote_path, local_dir), quite_mode) for f in
                        glob.glob("{}/**".format(local_dir), recursive=True))

        with ThreadPool(num_threads) as pool:
            pool.starmap(self.uploadfile, input_tuples)

    def download_files(self, remote_path, local_dir, num_threads=20, quite_mode=False):
        """
...
if "__main__" == __name__:
    ...
    parser.add_argument("--numthreads",
                        help="The local directory to save the file to", type=int, default=50)
    ...
    s3_util = S3Util()
    s3_util.download_files(args.s3url, args.localdir, num_threads=args.numthreads)
